...
- **Preconditions**: Admin JWT for tenant `T1`; no existing user with `newuser@example.com` in `T1`
- **Input**:
  ```json
  POST /users
...
- **Preconditions**: Admin JWT for tenant `T1`
- **Input**:
  ```json
  POST /users
  {
...
- **Category**: Nominal
- **Standard**: ISO 27001 A.9.2.1
- **Preconditions**: User `U1` exists in tenant `T1`; admin JWT for `T1`
- **Input**:
  ```
  GET /users/<U1-uuid>
  Authorization: Bearer <admin-jwt-T1>
...
- **Category**: Nominal
- **Preconditions**: Tenant `T1` has 5 users; admin JWT for `T1`
- **Input**:
  ```
  GET /users
  Authorization: Bearer <admin-jwt-T1>
...
- **Category**: Nominal
- **Preconditions**: User `U1` exists in tenant `T1` with email `old@example.com`
- **Input**:
  ```json
  PUT /users/<U1-uuid>
  {
...
- **Preconditions**: User `U1` has roles `["user"]`
- **Input**:
  ```json
  PUT /users/<U1-uuid>
  {
...
- **Category**: Nominal
- **Preconditions**: User `U1` is inactive (`is_active = false`)
- **Input**:
  ```json
  PUT /users/<U1-uuid>
  {
...
- **Preconditions**: User `U1` exists and is active in tenant `T1`
- **Input**:
  ```
...
- **Category**: Nominal
- **Preconditions**: User `U1` has roles `["user", "editor"]` and email `old@example.com`
- **Input**:
  ```json
  PUT /users/<U1-uuid>
  {
...
- **Category**: Nominal
- **Preconditions**: User `U1` with email `same@example.com`
- **Input**:
  ```json
  PUT /users/<U1-uuid>
  {
...
- **Category**: Edge Case
- **Preconditions**: User with `existing@example.com` exists in tenant `T1`
- **Input**:
  ```json
  POST /users
...
- **Category**: Edge Case
- **Preconditions**: User `shared@example.com` exists in tenant `T1`; admin JWT for tenant `T2`
- **Input**:
  ```json
  POST /users
...
- **Category**: Edge Case
- **Preconditions**: User `user@example.com` exists in tenant `T1`
- **Input**:
  ```json
  POST /users
...
- **Category**: Edge Case
- **Preconditions**: User `U2` exists with email `taken@example.com` in tenant `T1`
- **Input**:
  ```json
  PUT /users/<U1-uuid>
...
- **Category**: Security
- **Standard**: ISO 27001 A.9.4.1
- **Preconditions**: JWT with role `["user"]` only (no `admin`)
...
- **Category**: Security
- **Standard**: SOC 2 CC6.1
- **Preconditions**: Tenant `T1` has users; tenant `T2` has different users; admin JWT for `T2`
...
- **Category**: Security
- **Standard**: SOC 2 CC6.1
- **Preconditions**: RLS policies active on `users` table
