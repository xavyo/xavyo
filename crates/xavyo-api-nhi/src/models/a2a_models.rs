...
impl A2aErrorResponse {
    ...
    /// Target agent not active error.
    #[must_use]
    pub fn target_not_active() -> Self {
        Self::new("target_not_active", "Target agent is not in active state")
    }

    /// Source agent not active error.
    #[must_use]
    pub fn source_not_active() -> Self {
        Self::new("source_not_active", "Source agent is not in active state")
    }

    /// Permission denied error (no call permission).
    #[must_use]
    pub fn permission_denied() -> Self {
        Self::new(
            "forbidden",
            "Source agent does not have 'call' permission on target agent",
        )
    }
...
mod tests {
    ...
    fn test_error_responses() {
        ...
        let perm_err = A2aErrorResponse::permission_denied();
        assert_eq!(perm_err.error, "forbidden");
    }
}
