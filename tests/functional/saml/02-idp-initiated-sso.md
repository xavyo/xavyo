...
- **Category**: Nominal
- **Preconditions**: Same as TC-SAML-IDP-001
- **Input**:
  ```json
  POST /saml/initiate/<sp_uuid>
  Authorization: Bearer <jwt_token>
...
- **Preconditions**: Standard SP, authenticated user
...
- **Category**: Nominal
- **Preconditions**: SP configured with `sign_assertions=true`, tenant has active certificate
- **Input**: Valid IdP-initiated SSO request
...
- **Category**: Nominal
- **Preconditions**: SP with `include_groups=true`, user is member of groups `["engineering", "admin"]`
- **Input**: Valid IdP-initiated SSO request
...
- **Category**: Nominal
- **Preconditions**: SP with `include_groups=true`, `omit_empty_groups=true`, user has no group memberships
- **Input**: Valid IdP-initiated SSO request
...
- **Category**: Nominal
- **Preconditions**: SP configured with `group_value_format="name"`, user in group with display_name `Engineering Team`
- **Input**: Valid IdP-initiated SSO request
...
- **Category**: Edge Case
- **Preconditions**: SP registered with empty `acs_urls=[]` (would require direct DB manipulation)
- **Input**: Valid IdP-initiated SSO request
- **Expected Output**:
  ```
...
- **Category**: Edge Case
- **Preconditions**: Tenant has no active certificate in `tenant_idp_certificates`
- **Input**: Valid IdP-initiated SSO request
- **Expected Output**:
  ```
  Status: 500 Internal Server Error
...
- **Category**: Edge Case
- **Standard**: SAML 2.0 Profiles 4.1.5
- **Preconditions**: SP registered with `acs_urls=["https://sp.example.com/acs-primary", "https://sp.example.com/acs-secondary"]`
