...
- **Preconditions**: Confidential client with `client_credentials` grant type exists
- **Input**:
  ```
  POST /oauth/token
...
- **Preconditions**: Confidential client with `client_credentials` grant exists
- **Input**:
  ```
  POST /oauth/token
  Content-Type: application/x-www-form-urlencoded
...
- **Preconditions**: Client has allowed scopes `["read", "write", "admin"]`
- **Input**:
  ```
  POST /oauth/token
  Authorization: Basic base64({client_id}:{client_secret})
...
- **Preconditions**: Successfully issued token from TC-OAUTH-CC-001
...
- **Preconditions**: Client credentials grant succeeds
...
- **Preconditions**: Client has scopes `["read", "write", "admin"]`
- **Input**:
  ```
  POST /oauth/token
  Authorization: Basic base64({client_id}:{client_secret})
...
- **Preconditions**: Two confidential clients exist
- **Input**:
  ```
  POST /oauth/token
...
- **Preconditions**: Client with secret containing colons (regenerated)
- **Input**:
  ```
...
- **Preconditions**: Valid client credentials request
- **Input**: Same as TC-OAUTH-CC-001
- **Expected Output**:
...
- **Preconditions**: Issuer URL configured (e.g., `https://idp.xavyo.com`)
- **Input**: Same as TC-OAUTH-CC-001
- **Expected Output**:
...
- **Preconditions**: Client has scope `openid` in allowed scopes
- **Input**:
  ```
...
- **Preconditions**: Client has `offline_access` in allowed scopes
- **Input**:
  ```
...
- **Category**: Edge Case
- **Standard**: RFC 6749 Section 3.3
- **Preconditions**: Client has scopes `["read", "write"]` only
- **Input**:
  ```
...
- **Preconditions**: Valid client auth
- **Input**:
  ```
  grant_type=password
  ```
...
- **Preconditions**: Client belongs to `TENANT_A`, request uses `TENANT_B`
- **Input**:
  ```
...
- **Preconditions**: Client was deactivated via `DELETE /admin/oauth/clients/{id}`
- **Input**:
  ```
...
- **Category**: Edge Case
- **Standard**: Platform-specific
- **Preconditions**: UUID is valid but not a real tenant
- **Input**:
  ```
...
- **Preconditions**: Valid client_id, incorrect secret
- **Input**:
  ```
...
- **Preconditions**: Public client (no secret) exists
- **Input**:
  ```
...
- **Category**: Security
- **Standard**: RFC 6749 Section 4.4.2
- **Preconditions**: Confidential client with `grant_types: ["authorization_code"]`
- **Input**:
  ```
...
- **Preconditions**: Token issued for `TENANT_A`
...
- **Preconditions**: Successfully issued token
...
- **Preconditions**: Various invalid requests
