...
- **Special Setup**: Lifecycle tests require existing users in various states (active, suspended, soft-deleted) within the test tenant
...
- **Preconditions**: Fixtures: `ADMIN_JWT`, `TEST_TENANT`. User `U1` is active in tenant `T1`
- **Input**:
  ```json
  PUT /users/<U1-uuid>
  { "is_active": false }
...
- **Preconditions**: Fixtures: `ADMIN_JWT`, `TEST_TENANT`. User `U1` with roles, email, custom attributes
...
- **Category**: Nominal
- **Preconditions**: Fixtures: `ADMIN_JWT`, `TEST_TENANT`. Tenant has active and suspended users
- **Input**: `GET /users`
...
- **Category**: Nominal
- **Standard**: ISO 27001 A.9.2.6
- **Preconditions**: Fixtures: `ADMIN_JWT`, `TEST_TENANT`. User `U1` has `lifecycle_state_id` referencing a `gov_lifecycle_states` row named "Active"
...
- **Category**: Nominal
- **Preconditions**: Fixtures: `ADMIN_JWT`, `TEST_TENANT`. User `U1` has no `lifecycle_state_id`
- **Input**: `GET /users/<U1-uuid>`
...
- **Category**: Nominal
- **Preconditions**: Fixtures: `ADMIN_JWT`, `TEST_TENANT`. Some users have lifecycle states, others do not
- **Input**: `GET /users`
...
- **Category**: Nominal
- **Preconditions**: Fixtures: `ADMIN_JWT`, `TEST_TENANT`. User in terminal lifecycle state (e.g., "Deleted" with `is_terminal = true`)
- **Input**: `GET /users/<U1-uuid>`
...
- **Category**: Nominal
- **Preconditions**: Fixtures: `ADMIN_JWT`, `TEST_TENANT`. User `U1` created at time T0
...
- **Category**: Nominal
- **Preconditions**: Fixtures: `ADMIN_JWT`, `TEST_TENANT`. User `U1` starts active
- **Steps**:
...
- **Category**: Edge Case
- **Preconditions**: Fixtures: `ADMIN_JWT`, `TEST_TENANT`. User `U1` already has `is_active = true`
...
- **Category**: Edge Case
- **Preconditions**: Fixtures: `ADMIN_JWT`, `TEST_TENANT`. User `U1` already deactivated via `DELETE`
- **Input**: `DELETE /users/<U1-uuid>`
- **Expected Output**: Status 204 (idempotent success)

### TC-USER-LIFECYCLE-023: Update email on suspended user
- **Category**: Edge Case
- **Preconditions**: Fixtures: `ADMIN_JWT`, `TEST_TENANT`. User `U1` is suspended (`is_active = false`)
...
- **Category**: Edge Case
- **Preconditions**: Fixtures: `ADMIN_JWT`, `TEST_TENANT`. User `U1` is suspended
...
- **Category**: Edge Case
- **Preconditions**: Fixtures: `ADMIN_JWT`, `TEST_TENANT`. User exists in tenant
- **Input**:
  ```json
  PUT /users/<U1-uuid>
...
- **Category**: Edge Case
- **Preconditions**: Fixtures: `ADMIN_JWT`, `TEST_TENANT`. User has `lifecycle_state_id` pointing to "Active" state
...
- **Category**: Edge Case
- **Preconditions**: Fixtures: `ADMIN_JWT`, `TEST_TENANT`. Users U1 and U2 exist and are active
...
- **Category**: Security
- **Standard**: ISO 27001 A.9.2.6
- **Preconditions**: Fixtures: `ADMIN_JWT`, `TEST_TENANT`. Suspend user via `PUT /users/<U1> { "is_active": false }`
...
- **Category**: Security
- **Standard**: ISO 27001 A.9.2.6
- **Preconditions**: Fixtures: `ADMIN_JWT`, `TEST_TENANT`. Soft delete user via `DELETE /users/<U1>`
...
- **Category**: Security
- **Standard**: SOC 2 CC6.1
- **Preconditions**: Fixtures: `ADMIN_JWT`, `TEST_TENANT`. User exists in tenant `T1`; webhook endpoint configured
...
- **Preconditions**: Fixtures: `ADMIN_JWT`, `TEST_TENANT`. User exists in active state
...
- **Preconditions**: Fixtures: `ADMIN_JWT`, `TEST_TENANT`. Active user exists
...
- **Preconditions**: Fixtures: `ADMIN_JWT`, `TEST_TENANT`. Active user exists with roles and custom attributes
