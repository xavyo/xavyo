//! Revoke sessions and refresh tokens after identity or permission changes.
//!
//! Session-only revoke left opaque and OAuth refresh tokens valid, so the user
//! could mint a new access token without re-authenticating.
