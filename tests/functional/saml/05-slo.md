...
- **Preconditions**: Session store initialized (InMemory or PostgreSQL)
...
- **Preconditions**: Session `_req_abc123` stored for tenant T1
...
- **Category**: Nominal
- **Preconditions**: Store 3 sessions: 2 expired (expires_at in past), 1 valid
...
- **Preconditions**: PostgreSQL-backed session store, session stored
...
- **Preconditions**: SSO flow completes successfully
...
- **Preconditions**: Session stored and consumed once
...
- **Category**: Edge Case
- **Preconditions**: Session with `expires_at` in the past (beyond grace period)
...
- **Category**: Edge Case
- **Preconditions**: Session with request_id `_req_dup` already stored
...
- **Preconditions**: Session with `expires_at` = 15 seconds ago (grace period is 30 seconds)
...
- **Category**: Edge Case
- **Preconditions**: Session with `expires_at` = 60 seconds ago (beyond 30-second grace)
...
- **Category**: Edge Case
- **Preconditions**: All sessions are still valid
- **Input**: `cleanup_expired()`
- **Expected Output**: Returns `0` (nothing deleted)

### TC-SAML-SLO-018: Cleanup with all sessions expired
- **Category**: Edge Case
- **Preconditions**: All stored sessions have `expires_at` in the distant past
- **Input**: `cleanup_expired()`
...
- **Category**: Edge Case
- **Preconditions**: PostgreSQL session store
...
- **Category**: Edge Case
- **Preconditions**: PostgreSQL session store with valid session
...
- **Preconditions**: Tenant A stores session with request_id `_req_shared`; Tenant B has no sessions
...
- **Preconditions**: Session consumed once
...
- **Category**: Security
- **Preconditions**: Expired session exists in store
...
- **Category**: Security
- **Preconditions**: T1 has session `_req_001`, T2 has session `_req_002`
- **Input**:
